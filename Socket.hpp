...
namespace Socket {
    ...
    private:
        string _message;
    public:
        explicit Exception(string error) { this->_message = std::move(error); }
    ...
    public:
        ...
        void send(const Ip &ip, Port port, const Data &data) const;

        Datagram receive() const;
    };
    ...
    void UDP::send(const Ip &ip, Port port, const Data &data) const {
        sockaddr_in address{};
        address.sin_family = AF_INET;
        address.sin_port = htons(port);
        inet_aton(ip.c_str(), &address.sin_addr);

        if (sendto(this->_socket_id, (void *) data.c_str(), data.length() + 1, 0, (struct sockaddr *) &address,
    ...
    Datagram UDP::receive() const {
        int size = sizeof(struct sockaddr_in);
        char *buffer = (char *) malloc(sizeof(char) * MAX_BUFFER);
        struct sockaddr_in address{};
        Datagram ret;

        if (recvfrom(this->_socket_id, (void *) buffer, MAX_BUFFER, 0, (struct sockaddr *) &address,
                     (socklen_t *) &size) == -1)
